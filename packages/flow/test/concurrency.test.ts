...
describe("Flow com execuções concorrentes", () => {
  it("registra duas runs distintas, cada uma com seus próprios passos", async () => {
    ...
    const publish = (e: ExecutionEvent) => history.record(e);

    // Um app por workflow, como no uso real; os dois publicam no mesmo Flow.
    const appA = Thena.create(fluxoLento(30, "A"), { log: publish });
    ...
    for (const run of runs) {
      const events = history.eventsOf(run.id)!;
      expect(events.every((e) => e.runId === run.id)).toBe(true);
      expect(events.filter((e) => e.depth === 0)).toHaveLength(2);
  ...
  it("uma run que falha não contamina o status da outra", async () => {
    ...
    const appOk = Thena.create(fluxoLento(20, "ok"), { log: publish });
